...
func configMap(args []string) error {
	...
	if len(positional) < 2 {
		return fmt.Errorf("usage: ata config map <directory> <workspace>")
	}
...
func configUnmap(args []string) error {
	...
	if len(positional) < 1 {
		return fmt.Errorf("usage: ata config unmap <directory>")
	}
...
func configUsage() error {
	return fmt.Errorf(`usage: ata config <subcommand>
